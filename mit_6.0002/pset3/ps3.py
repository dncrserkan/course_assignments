...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        if not isinstance(dirt_amount, int) or dirt_amount < 0:
            raise ValueError("Dirt amount must be positive integer or zero")
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        return sum(1 for value in self.tiles.values() if value == 0)
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        if not isinstance(room, RectangularRoom):
            raise ValueError("Room must be type of RectangularRoom")
        self.room = room
        ...
        if not isinstance(capacity, int) or capacity <= 0:
            raise ValueError("Capacity must be postive integer")
    ...
    def set_robot_position(self, position):
        ...
        if not isinstance(position, Position):
            raise TypeError("position must be Position object")
    ...
    def set_robot_direction(self, direction):
        ...
        """
        if isinstance(direction, float) and 0 <= direction <= 360:
            self.direction = direction
        else:
            raise ValueError("Direction must be number between 0 and 360")
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        target = self.get_robot_position().get_new_position(self.get_robot_direction(), self.speed)
        if self.room.is_position_valid(target):
            self.set_robot_position(target)
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        target = self.get_robot_position().get_new_position(self.get_robot_direction(), self.speed)
        if self.room.is_position_valid(target) and not self.gets_faulty():
            self.set_robot_position(target)
            self.room.clean_tile_at_position(self.position, self.capacity)
        else:
            self.set_robot_direction(random.uniform(0, 360))
...
# OUTPUT
    # Standard Robot
    # avg time steps: 2065.12
    # Faulty Robot
    # avg time steps: 2492.89
#
#   Standard Robot has loer average time steps, so it's efficiency is higher than Faluty Robot.
#
#
#
#
...
#       robot cleans 80% of rooms with dimensions 10x30, 20x15, 25x12, and 50x6?
# 
# width = [10, 20, 25, 50]
# height = [30, 15, 12, 6]
# for i in range(len(width)):
#     print("Standard Robot")
#     print ('avg time steps for', width[i], "x", height[i], ":", str(run_simulation(1, 1.0, 1, width[i], height[i], 3, 0.8, 100, StandardRobot)))
#     print("Faulty Robot")
#     print ('avg time steps for', width[i], "x", height[i], ":", str(run_simulation(1, 1.0, 1, width[i], height[i], 3, 0.8, 100, FaultyRobot)))
#OUTPUT
    #Standard Robot
    # avg time steps for 10 x 30 : 1593.27
    # Faulty Robot
    # avg time steps for 10 x 30 : 1971.44
    # Standard Robot
    # avg time steps for 20 x 15 : 1567.8
    # Faulty Robot
    # avg time steps for 20 x 15 : 1918.92
    # Standard Robot
    # avg time steps for 25 x 12 : 1584.14
    # Faulty Robot
    # avg time steps for 25 x 12 : 1907.44
    # Standard Robot
    # avg time steps for 50 x 6 : 1704.9
    # Faulty Robot
    # avg time steps for 50 x 6 : 2423.91
#
#
#   In all cases, the "Standard Robot" outperforms the "Faulty Robot" in terms of cleaning efficiency.
#   The lower average time steps suggest that the standard robot is more effective in cleaning the 
#   environment compared to the faulty robot across various room sizes.
#

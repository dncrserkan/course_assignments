...
import struct
import sys
from helpers import BF, BI, RGBTRIPLE, sepia, grayscale, blur, reflect


def main():

    parser = argparse.ArgumentParser(description="Program accepts a bmp file and gives a new bmp file according to optional arguments")
    parser.add_argument("infile", help="Name of input file")
    parser.add_argument("outfile", help ="Name of output file")
    parser.add_argument("-b", "--blur", action="store_true", help="blur effect")
    parser.add_argument("-g", "--grayscale", action="store_true", help="turn grayscale")
    parser.add_argument("-r", "--reflect", action="store_true", help="reflect horizontal")
    parser.add_argument("-s", "--sepia", action="store_true", help="sepia effect")
    args = parser.parse_args()
    
    # Check filter validity
    counter = sum([args.blur, args.grayscale, args.reflect, args.sepia]) 
    if counter == 0:
    ...
    if bf.Type != int(0x4d42) or bf.OffBits != 54 or bi.Size != 40 \
                    or bi.BitCount != 24 or bi.Compression != 0:
        inptr.close()
        outptr.close()
    ...
    padding = (4 - (width * 3) % 4) % 4     # 3 is sizeof(RGBTRIPLE) as bytes
